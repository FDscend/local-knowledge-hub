// Project-maintained fork of obsidian-clipper-cn v1.4.6 src/api.ts (MIT).
// Fixed upstream: .ref/obsidian-clipper-cn/src/api.ts. Changes vs upstream:
//   1. Pass the full Document to DefuddleClass (defuddle 0.19 requires it).
// Bundled by `npm run build:vendor-clipper` into src/vendor/obsidian-clipper-cn/api.mjs.
...
export async function clip(options: ClipOptions): Promise<ClipResult> {
	...
	// Project fork: pass the full Document to DefuddleClass. Upstream passes
	// documentElement, which works with defuddle 0.16 but yields empty content
	// with defuddle 0.19 (it expects a complete Document). Defuddle 0.19 is
	// pinned because it explicitly supports linkedom (the Node DOM parser).
